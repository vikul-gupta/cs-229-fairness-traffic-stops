function error_result_vector = ComputeTestAccuracyEditedNEW(numtest, test_data, std_test_data, thetas_vec, true_output_test, threshold_vector)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%Compute Test Logistic Loss
testloss = 0;
        testloss = testloss - norm(thetas_vec)^2;
        for k=1:numtest
            
            %This is an ad-hoc thing to avoid some overflows
            temp1 = 1/(1+exp(-dot(thetas_vec.',std_test_data(k,:))));
            temp2 = 1-temp1;
            ...
            testloss = testloss + true_output_test(k)*temp1 ...
                  + (1-true_output_test(k))*temp2;
        end
        
%%%%%%%%%%%%%%%%%%%%%%%%%%Compute Individual Fairness Loss
 
whiteOutcome = test_data(:,white_index);
blackOutcome = test_data(:, black_index);
hispanicOutcome = test_data(:, hispanic_index);
asianOutcome = test_data(:, asian_index);
maleOutcome = test_data(:, male_index);
femaleOutcome = test_data(:, female_index);
age1625Outcome = test_data(:,index_1625);
age2635Outcome = test_data(:,index_2635);
age3645Outcome = test_data(:,index_3645);
age4655Outcome = test_data(:,index_4655);
age5665Outcome = test_data(:,index_5665);
age6675Outcome = test_data(:,index_6675);
age7685Outcome = test_data(:,index_7685);
ageAbove86Outcome = test_data(:,index_86plus);
raceOutcome = [whiteOutcome,blackOutcome,hispanicOutcome,asianOutcome];
genderOutcome = [maleOutcome,femaleOutcome];
ageOutcome = [age1625Outcome,age2635Outcome, age3645Outcome,age4655Outcome,age5665Outcome,age6675Outcome,...
                                                        age7685Outcome,ageAbove86Outcome];
...
for i=1:testnum
    for j=i+1:testnum
        if (true_output_test(i) == true_output_test(j)) && (raceOutcome(i) ~= raceOutcome(j))
            counter = counter +1;
            racesum = racesum + (dot(thetas_vec,std_test_data(i,:).')-dot(thetas_vec,std_test_data(j,:).'))^2;
        end
        
        if (true_output_test(i) == true_output_test(j)) && (genderOutcome(i) ~= genderOutcome(j))
            counter2 = counter2 +1;
            gendersum = gendersum + (dot(thetas_vec,std_test_data(i,:).')-dot(thetas_vec,std_test_data(j,:).'))^2;
        end
        
         if (true_output_test(i) == true_output_test(j)) && (ageOutcome(i) ~= ageOutcome(j))
            counter3 = counter3 +1;
            agesum = agesum + (dot(thetas_vec,std_test_data(i,:).')-dot(thetas_vec,std_test_data(j,:).'))^2;
         end
        
    end
end
raceIndFairness = 1/counter * racesum;
genderIndFairness = 1/counter2 * gendersum;
ageIndFairness = 1/counter3 * agesum;
 
        
%%%%%%%%%%%%%%%%%%%%%%%Compute Test Group Fairness Loss
...
for i=1:testnum
    for j=i+1:testnum
        if (true_output_test(i) == true_output_test(j)) && (raceOutcome(i) ~= raceOutcome(j))
            counter = counter +1;
            racesum = racesum + (dot(thetas_vec,std_test_data(i,:).')-dot(thetas_vec,std_test_data(j,:).'));
        end
        
        if (true_output_test(i) == true_output_test(j)) && (genderOutcome(i) ~= genderOutcome(j))
            counter2 = counter2 +1;
            gendersum = gendersum + (dot(thetas_vec,std_test_data(i,:).')-dot(thetas_vec,std_test_data(j,:).'));
        end
        
        if (true_output_test(i) == true_output_test(j)) && (ageOutcome(i) ~= ageOutcome(j))
            counter3 = counter3 +1;
            agesum = agesum + (dot(thetas_vec,std_test_data(i,:).')-dot(thetas_vec,std_test_data(j,:).'));
        end
    end
end
raceGroupFairness = (1/counter * racesum)^2;
genderGroupFairness = (1/counter2 * gendersum)^2;
ageGroupFairness = (1/counter3 * agesum)^2;
...
F186plus =0;
    ...
    error_result_vector = [misclassificationstest, count2test, testacc, nonWarnPercGENERAL, nonWarnPercWhite, nonWarnPercBlack, ...
        ...
        balancedError5665, balancedError6675, balancedError7685, balancedError86plus, testloss,raceIndFairness,genderIndFairness,ageIndFairness,raceGroupFairness...
        genderGroupFairness,ageGroupFairness];
...
46 - LogisticLoss
47- raceIndFairness
48- genderIndFairness
49- ageIndFairness
50 - raceGroupFairness
51- genderGroupFairness
52- ageGroupFairness
        %}
